        ...
        alarms.forEach(alarm => {
            if (alarm.enabled) {
                if (alarm.h === currentH_24 && alarm.m === currentM_24) {
                    lastAlarmMinute = currentTotalMinutes; // 発火済みとして記録
...
function setupNControl() {
    ...
    if (slider) {
        slider.oninput = (e) => {
            ...
            saveAppSettings(); // 改良点: スライダー変更時に保存
            updateClock();
        };
    }
}
